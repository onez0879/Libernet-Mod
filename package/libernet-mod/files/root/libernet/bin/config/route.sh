...
log() {
    "$LIBERNET_DIR/bin/log.sh" -w "[ROUTING] $1"
}
...
########################################
# active mihomo config
########################################

case "$MODE" in
    v2ray)
        MIHOMO_CFG="$LIBERNET_DIR/config/mihomo-tun.yaml"
        ;;
    *)
        MIHOMO_CFG="$LIBERNET_DIR/config/mihomo.yaml"
        ;;
esac
########################################
# active profile
...
ssh)
    ...
    if [ -f "$PROFILE_CFG" ]; then
        SERVER_IP="$(jq -r '.ip // empty' "$PROFILE_CFG")"
        PROXY_IP="$(jq -r '.http.proxy.ip // empty' "$PROFILE_CFG")"
    fi
...
delete_hooks() {
    ...
    while iptables -t nat -C OUTPUT -p tcp -j $LIBERNET_CHAIN 2>/dev/null; do
        iptables -t nat -D OUTPUT -p tcp -j $LIBERNET_CHAIN
    done

    while iptables -t nat -C PREROUTING -i br-lan -p tcp -j $LIBERNET_CHAIN 2>/dev/null; do
        iptables -t nat -D PREROUTING -i br-lan -p tcp -j $LIBERNET_CHAIN
    done

    while iptables -t nat -C OUTPUT -p udp --dport 53 -j $LIBERNET_DNS_CHAIN 2>/dev/null; do
        iptables -t nat -D OUTPUT -p udp --dport 53 -j $LIBERNET_DNS_CHAIN
    done

    while iptables -t nat -C OUTPUT -p tcp --dport 53 -j $LIBERNET_DNS_CHAIN 2>/dev/null; do
        iptables -t nat -D OUTPUT -p tcp --dport 53 -j $LIBERNET_DNS_CHAIN
    done

    while iptables -t nat -C PREROUTING -i br-lan -p udp --dport 53 -j $LIBERNET_DNS_CHAIN 2>/dev/null; do
        iptables -t nat -D PREROUTING -i br-lan -p udp --dport 53 -j $LIBERNET_DNS_CHAIN
    done

    while iptables -t nat -C PREROUTING -i br-lan -p tcp --dport 53 -j $LIBERNET_DNS_CHAIN 2>/dev/null; do
        iptables -t nat -D PREROUTING -i br-lan -p tcp --dport 53 -j $LIBERNET_DNS_CHAIN
    done
