...
function check_connection() {
    ...
    while [ $counter -lt $max_retries ]; do
        ...
        "${LIBERNET_DIR}/bin/log.sh" -w "Checking SOCKS connection ($((counter+1))/$max_retries)"

        if ! netstat -ln 2>/dev/null | grep -q "127.0.0.1:${DYNAMIC_PORT}"; then
            counter=$((counter+1))
            continue
        fi
        ...
        if curl \
            ...
            -fs https://ifconfig.me >/dev/null
        then

            CONNECTED=true

            "${LIBERNET_DIR}/bin/log.sh" \
            -w "<span style=\"color:green\">SOCKS connection available</span>"
    ...
    "${LIBERNET_DIR}/bin/log.sh" \
    -w "<span style=\"color:red\">SOCKS connection unavailable</span>"
