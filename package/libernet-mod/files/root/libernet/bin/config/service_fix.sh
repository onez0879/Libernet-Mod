...
MEMORY_CLEANER="$(jq -r '.system.memory_cleaner' "${SYSTEM_CONFIG}")"
PING_LOOP="$(jq -r '.tunnel.ping_loop' "${SYSTEM_CONFIG}")"
AUTO_RECON="$(jq -r '.tunnel.auto_recon' "${SYSTEM_CONFIG}")"
...
function check_connection() {
  ...
  while [ "$counter" -lt "$max_retries" ]; do
    ...
    "${LIBERNET_DIR}/bin/log.sh" -w "Checking SOCKS, attempt: $((counter+1))"
    echo "Checking SOCKS, attempt: $((counter+1))"

    # cek dulu apakah ssh sudah listen di port socks
    if netstat -lnt 2>/dev/null | grep -q "127.0.0.1:${DYNAMIC_PORT}"; then

      # tes trafik lewat socks
if curl \
--connect-timeout 5 \
--max-time 10 \
--socks5-hostname "127.0.0.1:${DYNAMIC_PORT}" \
http://clients3.google.com/generate_204 \
-o /dev/null 2>&1; then

        "${LIBERNET_DIR}/bin/log.sh" -w "<span style=\"color: green\">Socks connection available</span>"
        echo "Socks connection available!"
        # write connected timestamp
  ...
  "${LIBERNET_DIR}/bin/log.sh" -w "<span style=\"color: red\">Socks connection unavailable</span>"
  echo "Socks connection unavailable!"
